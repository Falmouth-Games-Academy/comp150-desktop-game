...
"""Main function that goes through different processes in order to create
a new random map for the player to explore.
"""
...
def generate_a_map():
    ...
    # This is what creates the map matrix array by using the numpy.random function
    ...
    # This for loop goes through each number in the map matrix array and
    # applies rules to it.
    for row_num, row_list in enumerate(map_matrix):
        for tile_num in enumerate(row_list):

            # This gets the position of the tile above and the tile to the left.
            tile_left = (row_num, tile_num[0] - 1)
            current_pos = (row_num, tile_num[0])

            # IF statement rules to apply to the map matrix.
            # These IF statements are what makes the random map look a certain way.
    ...
    # These lists need to be deleted before creating a new map so the
    # old colliders don't stick around.
    del wall_list[:]
    del grav_well_list[:]
    del laser_list[:]
    del win_tile_list[:]
